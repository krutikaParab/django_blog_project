...
def user_change(request):
    ...
    form = UserProfileChange(instance=currrent_user)
    if request.method == 'POST':
        form = UserProfileChange(request.POST, instance=currrent_user)
        if form.is_valid():
            form.save()
            form = UserProfileChange(instance=currrent_user)
...
def password_change(request):
    currrent_user = request.user
    changed = False
    form = PasswordChangeForm(currrent_user)
    if request.method == 'POST':
        form = PasswordChangeForm(currrent_user, data = request.POST)
        if form.is_valid():
            form.save()
            changed=True
